...
def test_compare_returns_status_ok():
    ...
    from delegator.models import DelegationResult
    ...
    with patch("delegator.dashboard.api.execute", return_value=mock_result):
...
def test_compare_handles_execute_exception():
    ...
    with patch("delegator.dashboard.api.execute", side_effect=RuntimeError("agent unavailable")):
